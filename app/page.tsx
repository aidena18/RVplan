...
const steps = [
  ["Club details", "Tell us whoâ€™s planning"],
  ["Event basics", "Date, time & attendance"],
  ["Space & setup", "Find the right room"],
  ["Food & supplies", "Plan what youâ€™ll need"],
...
const fieldLabels: Record<string, string> = {
  ...
  advisorName: "Club advisorâ€™s name",
  advisorEmail: "Advisorâ€™s email",
...
export default function Home() {
  ...
  const [clubSetup, setClubSetup] = useState({
    officialName: "", shortName: "", category: "Academic", description: "", mission: "", status: "New", year: "2026â€“2027", semester: "Fall 2026", activeMembers: "", interestedMembers: "", audience: "", email: "", social: "", clubPage: "", meetingFrequency: "Weekly", meetingLocation: "", recognized: "I'm not sure",
    ...
    <main className={`app view-${view} coach-${coachMode}`}>
      <header className="hero">
        <nav className="topbar" aria-label="Main navigation">
          <button className="brand homeBrand" onClick={() => setView("home")}>
            ...
            <span><strong>RVplan</strong><small>Raritan Valley Â· Student Life</small></span>
          </button>
          <div className="navActions">
            ...
            <button className="helpButton" onClick={() => setHelpOpen(true)}><Icon>âœ¦</Icon> Talk to Student Life</button>
          </div>
        </nav>
        <div className="heroCopy">
          <p className="eyebrow">RVplan Â· {view === "home" ? "Tools for student club leaders" : view === "student" ? "Student club event request" : view === "coach" ? "AI-powered club leadership coach" : "Student Life staff demo"}</p>
          ...
          <p>{view === "home" ? "Plan an event or organize your clubâ€™s semester." : view === "student" ? "Answer a few questions. Get a clear event checklist." : view === "coach" ? "Set goals, organize officers, and plan the semester." : "Review club requests and see what needs attention."}</p>
        </div>
      ...
      {view === "home" ? <section className="homeWorkspace" aria-label="RVplan tools">
        ...
        <div className="toolChoices">
          <div className="homeToolGroup"><button className="toolCard eventTool" onClick={() => { setStep(0); setView("student"); }}><span className="toolIcon">âœ¦</span><div><small>EVENT PLANNER</small><h3>Plan an event</h3><p>Answer a few questions to get guidance for your room, food, budget, and approvals.</p><strong>Start planning <b>â†’</b></strong></div></button><div className="demoLinks"><button onClick={() => setView("staff")}>Staff demo</button></div></div>
          <div className="homeToolGroup"><button className="toolCard coachTool" onClick={() => { setSetupStep(0); setCoachMode("setup"); setView("coach"); }}><span className="toolIcon">â—Ž</span><div><small>CLUB LEADERSHIP</small><h3>AI Leadership Coach</h3><p>Get personalized guidance to organize officers, set goals, and build your semester plan.</p><strong>Start coaching <b>â†’</b></strong></div></button><div className="demoLinks"><button onClick={() => { setCoachMode("samples"); setView("coach"); }}>Browse sample clubs</button><button onClick={() => { setCoachMode("advisor"); setView("coach"); }}>Faculty advisor demo</button></div></div>
        </div>
            ...
            {steps.map(([title], index) => (
              <li key={title} className={index === step ? "active" : index < step ? "done" : ""}>
                <button onClick={() => jumpTo(index)} disabled={index > step} aria-current={index === step ? "step" : undefined}>
                  <span>{index < step ? "âœ“" : index + 1}</span>{title}
                </button>
              </li>
            ))}
          </ol>
        ...
        <div className="contentGrid">
          <section className="formCard">
            {step === 0 && <>
              <div className="sectionHeading"><span className="sectionIcon">â™£</span><div><p>First things first</p><h2>Letâ€™s start with your club.</h2><span>This helps Student Life know who to follow up with.</span></div></div>
              <div className="formGrid">
                ...
                {input("advisorName", "text", "Advisorâ€™s full name")}
                {input("advisorEmail", "email", "advisor@raritanval.edu")}
              </div>
            </>}

            {step === 1 && <>
              <div className="sectionHeading"><span className="sectionIcon">âŒ</span><div><p>Shape the idea</p><h2>Tell us about your event.</h2><span>An estimate is fineâ€”you can confirm details with staff later.</span></div></div>
              <div className="formGrid">
                ...
                <label className="field guestSpeakerField"><span>Guest speaker</span><span className={`guestSpeakerControl ${data.guestSpeaker ? "selected" : ""}`}><input type="checkbox" checked={data.guestSpeaker} onChange={(e) => update("guestSpeaker", e.target.checked)} /><span className="guestCheck">âœ“</span><strong>{data.guestSpeaker ? "Guest speaker included" : "Include a guest speaker"}</strong></span></label>
                {input("date", "date")}
                {input("attendees", "number")}
                {input("startTime", "time")}
                {input("endTime", "time")}
                <label className="field"><span>Do you plan to collect RSVPs?</span><select value={data.rsvpPlan} onChange={(e) => update("rsvpPlan", e.target.value)}><option>Yes, weâ€™ll send an RSVP</option><option>No, this will be open attendance</option><option>Not sure yet</option></select><em>For larger events, we recommend collecting RSVPs so you can plan the room, food, and supplies.</em></label>
                <label className="field full"><span>Extra event details</span><textarea value={data.eventDetails} onChange={(e) => update("eventDetails", e.target.value)} placeholder="Share the theme, planned activities, special guests, travel plans, or anything else Student Life should knowâ€¦" /></label>
              </div>
            </>}

            {step === 2 && <>
              <div className="sectionHeading"><span className="sectionIcon">âŒ‚</span><div><p>Choose your setting</p><h2>What kind of space works best?</h2><span>Weâ€™ll turn your answers into a room recommendation.</span></div></div>
              <div className="choiceGrid">
                {["Classroom-style room", "Open lounge / social space", "Large gathering space", "Outdoor space"].map((option) => <label className={`choiceCard ${data.locationType === option ? "selected" : ""}`} key={option}><input type="radio" name="locationType" checked={data.locationType === option} onChange={() => update("locationType", option)} /><span className="choiceMark">{option.includes("Classroom") ? "â–¤" : option.includes("Outdoor") ? "â˜€" : option.includes("Large") ? "â—Ž" : "â—«"}</span><strong>{option}</strong></label>)}
              </div>
              <h3 className="miniHeading">What does the room need?</h3>
              <div className="checkGrid">
                {["Projector / screen", "Microphones", "Moveable tables", "Extra chairs", "Stage or performance area", "Power access"].map((option) => <label key={option}><input type="checkbox" checked={data.roomNeeds.includes(option)} onChange={() => toggleArray("roomNeeds", option)} /><span>âœ“</span>{option}</label>)}
              </div>
              <label className="field full"><span>Accessibility or setup notes</span><textarea value={data.accessibility} onChange={(e) => update("accessibility", e.target.value)} placeholder="Wheelchair access, quiet space, special seating, loading needsâ€¦" /></label>
              <div className="recommendation"><Icon>â˜…</Icon><div><small>YOUR ROOM MATCH</small><strong>{roomRecommendation}</strong><p>Student Life will confirm availability and the exact room.</p></div></div>
            </>}

            {step === 3 && <>
              <div className="sectionHeading"><span className="sectionIcon">â—’</span><div><p>Feed the crowd</p><h2>Food, drinks, and supplies.</h2><span>Weâ€™ll estimate quantities from your expected attendance.</span></div></div>
              <div className="choiceGrid three foodChoices">
                {["No food needed", "Pizza", "Snacks & drinks", "Order from the cafeteria", "Other food"].map((option) => <label className={`choiceCard ${data.food === option ? "selected" : ""}`} key={option}><input type="radio" name="food" checked={data.food === option} onChange={() => update("food", option)} /><span className="choiceMark">{option === "Pizza" ? "â—”" : option.includes("Snacks") ? "â–£" : option.includes("cafeteria") ? "â–¤" : option === "Other food" ? "+" : "â€”"}</span><strong>{option}</strong></label>)}
              </div>
              ...
              <div className="checkGrid">
                {["Vegetarian", "Vegan", "Gluten-free", "Halal", "Kosher", "Nut allergy"].map((option) => <label key={option}><input type="checkbox" checked={data.dietary.includes(option)} onChange={() => toggleArray("dietary", option)} /><span>âœ“</span>{option}</label>)}
              </div>
              <label className="field full topGap"><span>Anything else you need?</span><input value={data.notes} onChange={(e) => update("notes", e.target.value)} placeholder="Decorations, signs, gamesâ€¦" /></label>
            </>}
            ...
            {step === 4 && <>
              ...
              {budgetAmount === 0 && <div className="noBudget"><Icon>âœ“</Icon><div><strong>No budget request needed</strong><p>You can continue to your plan. Change the amount above if your event needs funding.</p></div></div>}
              {budgetAmount > 0 && <div className="budgetRequest">
                <div className="budgetFormHeader"><span>RVplan Â· RVCC Student Life</span><strong>Club / Organization Budget Proposal</strong><p>Events & Projects</p></div>
                ...
                <div className="preloadedGrid">
                  ...
                  <div><small>Event time</small><strong>{data.startTime}â€“{data.endTime}</strong></div><div><small>Semester & year</small><strong>{semester}</strong></div>
                  ...
                  <div><small>Primary contact</small><strong>{data.studentName} Â· {data.email}{data.phone ? ` Â· ${data.phone}` : ""}</strong></div><div><small>Club advisor</small><strong>{data.advisorName}{data.advisorEmail ? ` Â· ${data.advisorEmail}` : ""}</strong></div>
                ...
                <p className="sectionExplainer">List three skills students are expected to strengthen by participating in this eventâ€”for example, leadership, teamwork, public speaking, diplomacy, or academic knowledge.</p>
                ...
                <div className={`budgetBalance ${Math.abs(budgetAmount - budgetBreakdownTotal) < .01 ? "balanced" : ""}`}><span>Breakdown total <strong>${budgetBreakdownTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></span><span>{Math.abs(budgetAmount - budgetBreakdownTotal) < .01 ? "âœ“ Matches your estimate" : `$${Math.abs(budgetAmount - budgetBreakdownTotal).toFixed(2)} ${budgetBreakdownTotal < budgetAmount ? "left to assign" : "over estimate"}`}</span></div>
              </div>}
            </>}

            {step === 5 && <>
              <div className="sectionHeading"><span className="sectionIcon">âœ“</span><div><p>Your planning roadmap</p><h2>{submitted ? "Your request is ready." : "Review your event plan."}</h2><span>{submitted ? "Bring this plan to Student Life to start the official approval process." : "Check the details, then create your planning checklist."}</span></div></div>
              <div className="summaryHero"><div><small>{data.eventType.toUpperCase()}</small><h3>{data.eventName || "Your club event"}</h3><p>{data.clubName} Â· {data.date ? new Date(`${data.date}T12:00:00`).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) : "Date to be confirmed"} Â· {data.attendees} guests Â· {data.rsvpPlan}{data.guestSpeaker ? " Â· Guest speaker" : ""}</p></div><button onClick={() => setStep(1)}>Edit details</button></div>
              <div className="summaryGrid">
                <div><span className="summaryIcon">âŒ‚</span><small>SPACE</small><strong>{roomRecommendation}</strong><p>{data.roomNeeds.length ? data.roomNeeds.join(" Â· ") : "Standard room setup"}</p></div>
                <div><span className="summaryIcon">â—’</span><small>FOOD</small><strong>{data.food === "Pizza" ? `${pizzaCount} large pizzas` : data.food === "Snacks & drinks" ? "Snacks & drinks" : data.food === "Order from the cafeteria" ? "Cafeteria food order" : data.food === "Other food" ? data.foodOther || "Custom food order" : "No food requested"}</strong><p>{data.dietary.length ? `Plan for: ${data.dietary.join(", ")}` : "No dietary needs listed"}</p></div>
              </div>
              ...
              <div className="nextSteps">
                ...
                ].map(([number, title, detail]) => <div className="nextRow" key={number}><span>{number}</span><div><strong>{title}</strong><p>{detail}</p></div><b>â—‹</b></div>)}
              </div>
              {submitted && <div className="successNote"><Icon>âœ“</Icon><div><strong>Plan submitted</strong><p>Your plan has been submitted. Watch your RVCC email for next steps.</p></div></div>}
            </>}

            <div className="formFooter">
              <button className="backButton" onClick={() => setStep((current) => Math.max(0, current - 1))} disabled={step === 0}>â† Back</button>
              <span>Your information stays on this device.</span>
              {step < 5 ? <button className="primaryButton" onClick={next}>Continue to {steps[step + 1][0].toLowerCase()} <span>â†’</span></button> : <div className="finalActions"><button className="printButton" onClick={() => window.print()}>Print plan</button><button className="primaryButton" onClick={() => setSubmitted(true)}>{submitted ? "Plan submitted âœ“" : "Submit my plan â†’"}</button></div>}
            </div>
          </section>

        </div>
        <div className="compactHelpBar"><span className="helpIcon">?</span><div><strong>Need help planning?</strong><p>Student Life can help with rooms, funding, and approvals.</p></div><button onClick={() => setHelpOpen(true)}>Talk to staff <span>â†’</span></button></div>
        ...
        {coachMode === "setup" ? <ClubSetupWizard setup={clubSetup} setSetup={setClubSetup} step={setupStep} setStep={setSetupStep} onSave={saveClubDraft} onExit={() => { saveClubDraft(); setView("home"); }} onGenerate={() => setCoachMode("plan")} savedNotice={savedNotice} /> : coachMode === "advisor" ? <div className="advisorPortal">
          <div className="portalHeader"><button onClick={() => setView("home")}>â† Back to tools</button><div><p className="eyebrow dark">FACULTY ADVISOR PORTAL Â· DEMO</p><h2>Advisor review center</h2><span>Review club information, leave guidance, and request Student Life support.</span></div><button onClick={() => setCoachMode("setup")}>View club profile â†’</button></div>
          <div className="advisorStats"><article><span>CLUBS ADVISED</span><strong>2</strong><small>1 needs review</small></article><article><span>AWAITING REVIEW</span><strong>3</strong><small>Constitution Â· event Â· budget</small></article><article><span>UPCOMING EVENTS</span><strong>2</strong><small>Next event in 18 days</small></article><article><span>VACANT REQUIRED ROLES</span><strong>2</strong><small>Student follow-up needed</small></article></div>
          <div className="advisorLayout"><section><p className="eyebrow dark">ITEMS AWAITING REVIEW</p><h3>{clubSetup.officialName || "Philosophy Club"}</h3>{[["Constitution extraction", clubSetup.constitutionVerified ? "Student reviewed" : "Needs verification"], ["Cabinet assignments", `${clubSetup.officers.filter((item: any) => item.status === "Vacant").length} vacancies`], ["Financial snapshot", `$${clubSetup.balance || "175"} Â· ${clubSetup.balanceSource}`], ["Semester goals", `${clubSetup.goals.length || 4} selected`]].map(([label, detail]) => <div className="advisorReviewItem" key={label}><span><strong>{label}</strong><small>{detail}</small></span><button>Review</button><button>Comment</button></div>)}</section><aside><p className="eyebrow dark">COLLABORATION LOG</p><h3>Recent updates</h3><ul><li><b>Student</b><span>Updated cabinet vacancies<small>Today Â· 10:42 AM</small></span></li><li><b>Advisor</b><span>Requested a regular meeting schedule<small>Yesterday Â· 3:15 PM</small></span></li><li><b>Student Life</b><span>Balance marked demo data<small>Sep 3 Â· 11:20 AM</small></span></li></ul><textarea placeholder="Add advisor guidance or a correctionâ€¦" /><button>Add visible comment</button><button className="requestHelp">Request Student Life assistance</button></aside></div>
        </div> : <>
        {coachMode === "samples" && <div className="sampleBanner"><div><strong>Sample club profiles</strong><span>Explore different club situations and planning approaches.</span></div><button onClick={() => { setSetupStep(0); setCoachMode("setup"); }}>Set up my own club â†’</button></div>}
        <div className="coachTop"><button onClick={() => setView("home")}>â† Back to tools</button><span>PERSONALIZED CLUB PLAN</span></div>
        <div className="profileSwitcher compactProfile"><div><p className="eyebrow dark">CLUB PLAN</p><h2>{activeCoachContext.clubProfile}</h2><span>Personalized guidance based on this clubâ€™s saved information.</span></div>{coachMode === "samples" && <label><span>Sample club</span><select value={coachProfile} onChange={(event) => { setCoachProfile(event.target.value); setCoachResponse(""); }}>{demoClubContexts.map((club) => <option key={club.id} value={club.id}>{club.clubProfile}</option>)}</select></label>}</div>
        <div className="contextStrip"><span><small>CLUB STAGE</small><strong>{activeCoachContext.clubStage}</strong></span><span><small>MEMBERS</small><strong>{activeCoachContext.membershipMetrics.active} active / {activeCoachContext.membershipMetrics.interested} interested</strong></span><span><small>OFFICERS</small><strong>{activeCoachContext.officerRoster.length} active Â· {activeCoachContext.vacantRoles.length} vacant</strong></span><span><small>BUDGET</small><strong>${activeCoachContext.availableBudget}</strong></span><span><small>WEEKS LEFT</small><strong>{activeCoachContext.weeksRemaining}</strong></span></div>
        <div className="coachShell">
          ...
          <div className="coachResult">
            ...
            {coachTab === "growth" && <div className="growthView"><article><span>01</span><div><small>RECRUITMENT FUNNEL</small><h4>Move students from awareness to active ownership.</h4><p>{activeCoachOutput.recruitmentStrategy}</p><div className="funnel"><b>Awareness</b><i>â†’</i><b>Interest</b><i>â†’</i><b>First visit</b><i>â†’</i><b>Return</b><i>â†’</i><b>Active role</b><i>â†’</i><b>Leadership</b></div></div></article><article><span>02</span><div><small>RETENTION STRATEGY</small><h4>Address this clubâ€™s real participation gap.</h4><p>{activeCoachOutput.retentionStrategy}</p><div className="fitBox"><small>DIAGNOSTIC SIGNAL</small><p>{activeCoachContext.healthDiagnostic}</p></div></div></article></div>}
          </div>
          <button className="coachChatLauncher" onClick={() => setCoachChatOpen((open) => !open)} aria-expanded={coachChatOpen}><span>â—Ž</span><strong>{coachChatOpen ? "Close coach" : "Ask RVplan Coach"}</strong></button>
          {coachChatOpen && <aside className="coachSidePanel"><div className="chatHeader"><span>â—Ž</span><div><strong>RVplan Coach</strong><small>Personalized for {activeCoachContext.clubProfile}</small></div><button onClick={() => setCoachChatOpen(false)} aria-label="Close coach">Ã—</button></div><section className="askCoach"><div className="coachMessage">Hi! What would you like help with?</div><div className="questionChips">{["What should we focus on this month?", "Give us event ideas under $200."].map((question) => <button key={question} onClick={() => setCoachGoal(question)}>{question}</button>)}</div>{coachResponse && <div className="coachAnswer"><p>{coachResponse}</p></div>}<div className="chatComposer"><textarea value={coachGoal} onChange={(event) => setCoachGoal(event.target.value)} placeholder="Message your coachâ€¦" /><button onClick={() => setCoachResponse(activeCoachOutput.coachAnswer)} aria-label="Send message">â†’</button></div></section></aside>}
        </div>
        </>}
        ...
        <div className="sharedViewNote"><span>ðŸ‘</span><p><strong>{staffView === "all" ? "Shared team view" : `${staffView === "fran" ? "Franâ€™s" : staffView === "kiswah" ? "Kiswahâ€™s" : "Alayshaâ€™s"} clubs`}</strong> Everyone can view and help.</p></div>
        ...
        <div className="staffLayout">
          <section className="requestPanel">
            ...
            <div className="requestTable" role="table" aria-label="Student event requests">
              ...
              {[
                { event: data.eventName || "International Game Night", club: data.clubName || "Global Connections Club", owner: "fran", ownerName: "Fran", date: data.date || "Oct 24", needs: budgetAmount > 0 ? `Room Â· $${budgetAmount.toLocaleString()}` : "Room Â· Food", status: submitted ? "New submission" : "Needs review", tone: "review" },
                ...
                { event: "Fall Cultural Showcase", club: "Asian Student Association", owner: "kiswah", ownerName: "Kiswah", date: "Nov 7", needs: "Stage Â· Catering", status: "Room pending", tone: "pending" },
                { event: "Engineering Design Expo", club: "Engineering Club", owner: "kiswah", ownerName: "Kiswah", date: "Nov 12", needs: "Tables Â· Power", status: "Needs review", tone: "review" },
                { event: "Film Club Movie Night", club: "Film Club", owner: "alaysha", ownerName: "Alaysha", date: "Nov 14", needs: "Auditorium Â· AV", status: "Approved", tone: "approved" },
                { event: "Open Mic Night", club: "Music Club", owner: "alaysha", ownerName: "Alaysha", date: "Nov 20", needs: "Stage Â· Mics", status: "Room pending", tone: "pending" },
                { event: "Holiday Bake Sale", club: "Business Club", owner: "fran", ownerName: "Fran", date: "Dec 3", needs: "Tables Â· Budget", status: "Budget review", tone: "review" },
                { event: "Wellness Workshop", club: "Psychology Club", owner: "kiswah", ownerName: "Kiswah", date: "Dec 5", needs: "Classroom Â· Food", status: "Approved", tone: "approved" },
              ].filter((request) => staffView === "all" || request.owner === staffView).map((request) => <button className={`requestRow owner-${request.owner}`} role="row" key={request.event}><span><strong>{request.event}</strong><small>{request.club}<em className={`ownerTag ${request.owner}`}>{request.ownerName}</em></small></span><span>{request.date}</span><span>{request.needs}</span><span><b className={`status ${request.tone}`}>{request.status}</b><i>â†’</i></span></button>)}
            </div>
          </section>
          <aside className="staffSidebar">
            <section className={`personalTodos ${staffView}`}><p className="eyebrow dark">{staffView === "all" ? "TEAM PRIORITIES" : `${staffView.toUpperCase()}â€™S TO-DO LIST`}</p><h3>{staffView === "all" ? "Keep these moving" : "Assigned club follow-up"}</h3><ul>{(staffView === "fran" ? [["Review SGA budget", "Leadership Roundtable"], ["Confirm tables", "Business Club bake sale"], ["Check new submission", data.clubName || "Global Connections Club"]] : staffView === "kiswah" ? [["Confirm showcase room", "Asian Student Association"], ["Review power needs", "Engineering Club"], ["Send approval", "Psychology Club"]] : staffView === "alaysha" ? [["Confirm auditorium AV", "Film Club"], ["Reserve microphones", "Music Club"], ["Send student updates", "2 confirmations ready"]] : [["Review budget requests", "3 proposals are waiting"], ["Confirm event rooms", "4 requests need a location"], ["Send student updates", "2 confirmations are ready"]]).map(([task, detail], index) => <li key={task}><b>{index + 1}</b><span><strong>{task}</strong><small>{detail}</small></span></li>)}</ul></section>
        ...
        <section className="followUpQueue"><div className="panelHeading"><div><p className="eyebrow dark">CLUB SUPPORT & FOLLOW-UP</p><h3>{staffClubFilter}</h3></div><span>AI may suggest â€œpotential inactivity riskâ€; only authorized Student Life staff assigns an official status.</span></div><div className="followUpRows">{[
          { club: "Philosophy Club", status: "Restarting", reason: "Required cabinet vacancies Â· no recent event history", last: "Sep 8", advisor: "Advisor confirmed", balance: "$175 student reported", owner: "Fran", due: "Sep 22" },
          { club: "Desi Club", status: "Active but needs attention", reason: "Officer workload concern Â· major event budget pacing", last: "Sep 14", advisor: "Active", balance: "$1,850 verified demo", owner: "Alaysha", due: "Sep 29" },
          { club: "Engineering Club", status: "Active", reason: "Industry Outreach Chair vacant Â· project safety review", last: "Sep 16", advisor: "Technical review pending", balance: "$920 advisor reviewed", owner: "Kiswah", due: "Oct 2" },
        ].filter((club) => staffClubFilter === "All clubs" || staffClubFilter === "Needs attention" || club.status.includes(staffClubFilter.replace("Inactive", "inactive")) || (staffClubFilter === "Restarting" && club.status === "Restarting") || (staffClubFilter === "Officer vacancies" && club.reason.includes("vacant"))).map((club) => <article key={club.club}><div><span className={`clubStatus ${club.status.toLowerCase().replaceAll(" ", "-")}`}>{club.status}</span><h4>{club.club}</h4><p>{club.reason}</p></div><dl><dt>Last activity</dt><dd>{club.last}</dd><dt>Advisor</dt><dd>{club.advisor}</dd><dt>Balance</dt><dd>{club.balance}</dd><dt>Assigned staff</dt><dd>{club.owner}</dd><dt>Follow-up due</dt><dd>{club.due}</dd></dl><div><button>Open club â†’</button><select aria-label={`More actions for ${club.club}`} defaultValue=""><option value="" disabled>More actions</option><option>Send outreach email</option><option>Mark contacted</option><option>Add note</option><option>Schedule meeting</option></select></div></article>)}</div></section>
        <div className="demoNote"><Icon>i</Icon><div><strong>Demo workspace</strong><p>This staff dashboard uses sample requests for demonstration. Connect it to RVCCâ€™s official workflow before using it for live approvals.</p></div></div>
      </section>}

      <footer><div className="footerBrand"><img src="/rvcc-logo.png" alt="" /><span><strong>RVplan</strong><small>Raritan Valley Community College Â· Student Life</small></span></div><div><a href="mailto:studentlife@raritanval.edu">studentlife@raritanval.edu</a><a href="tel:+19082188873">908-218-8873</a></div></footer>

      {helpOpen && <div className="modalBackdrop" role="presentation" onMouseDown={() => setHelpOpen(false)}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="help-title" onMouseDown={(e) => e.stopPropagation()}><button className="modalClose" onClick={() => setHelpOpen(false)} aria-label="Close help dialog">Ã—</button><span className="helpIcon">?</span><p className="eyebrow dark">Real people, real help</p><h2 id="help-title">Talk to Student Life.</h2><p>Staff can help with rooms, approvals, funding, food, accessibility, promotion, and any question that comes up.</p><div className="contactOptions"><a href="mailto:studentlife@raritanval.edu?subject=Club%20event%20planning%20help"><Icon>âœ‰</Icon><span><strong>Email Student Life</strong>studentlife@raritanval.edu</span><b>â†’</b></a><a href="tel:+19082188873"><Icon>â˜Ž</Icon><span><strong>Call the office</strong>908-218-8873</span><b>â†’</b></a><div><Icon>âŒ‚</Icon><span><strong>Visit in person</strong>Bateman Student Center, 1st Floor</span></div></div><small>Office hours and staff availability may vary. Contact the office before visiting.</small></section></div>}
    </main>
  );
}
