...
export default function Home() {
  ...
  const [coachMode, setCoachMode] = useState<"welcome" | "setup" | "samples" | "plan" | "advisor">("welcome");
  ...
  return (
    <main>
      ...
      {view === "home" ? <section className="homeWorkspace" aria-label="RVplan tools">
        ...
        {coachMode === "welcome" ? <div className="coachWelcome">
          <button className="backToTools" onClick={() => setView("home")}>← Back to tools</button>
          <div className="welcomeMark">◎</div><p className="eyebrow dark">CLUB LEADERSHIP COACH</p><h2>Let’s set up your club.</h2><p>Your coach will use your club’s identity, constitution, leadership team, goals, finances, event history, and current challenges to build a personalized semester plan.</p>
          <div className="coachEntryChoices"><button className="primaryEntry" onClick={() => { setSetupStep(0); setCoachMode("setup"); }}><span>01</span><div><small>RECOMMENDED</small><h3>Create or set up my club</h3><p>Complete a guided eight-step setup. You can save progress and return later.</p><strong>Start club setup →</strong></div></button><button onClick={() => setCoachMode("samples")}><span>02</span><div><small>REVIEWER DEMO</small><h3>View sample clubs</h3><p>Compare three fictional profiles without completing onboarding.</p><strong>Browse sample clubs →</strong></div></button></div>
          <div className="roleAccess"><span>Supported roles</span><b>Student officer</b><b>Club president</b><b>Faculty advisor</b><b>Student Life staff</b><b>Platform administrator</b></div><button className="advisorDemoLink" onClick={() => setCoachMode("advisor")}>Open faculty advisor demo →</button>
        </div> : coachMode === "setup" ? <ClubSetupWizard setup={clubSetup} setSetup={setClubSetup} step={setupStep} setStep={setSetupStep} onSave={saveClubDraft} onExit={() => { saveClubDraft(); setCoachMode("welcome"); }} onGenerate={() => setCoachMode("plan")} savedNotice={savedNotice} /> : coachMode === "advisor" ? <div className="advisorPortal">
          <div className="portalHeader"><button onClick={() => setCoachMode("welcome")}>← Coach home</button><div><p className="eyebrow dark">FACULTY ADVISOR PORTAL · DEMO</p><h2>Advisor review center</h2><span>Review club information, leave guidance, and request Student Life support.</span></div><button onClick={() => setCoachMode("setup")}>View club profile →</button></div>
        ...
        <div className="profileSwitcher"><div><p className="eyebrow dark">DEMO PROFILE SWITCHER</p><h2>See the coach adapt to three different clubs.</h2><span>Switch profiles to compare priorities, roadmaps, events, officer plans, and coach answers.</span></div><div>{demoClubContexts.map((club) => <button key={club.id} className={coachProfile === club.id ? "active" : ""} onClick={() => { setCoachProfile(club.id); setCoachResponse(""); }}><strong>{club.clubProfile}</strong><small>{club.clubStage} · {club.membershipMetrics.active} active</small></button>)}</div></div>
        ...
        <div className="coachShell">
          ...
          <aside className="coachSidePanel"><section><p className="eyebrow dark">SAVED CLUB CONTEXT</p><h3>What the coach knows</h3><ul><li><span>Mission</span><strong>{activeCoachContext.clubMission}</strong></li><li><span>Weaknesses</span><strong>{activeCoachContext.identifiedWeaknesses.join(" · ")}</strong></li><li><span>Goals</span><strong>{activeCoachContext.semesterGoals.join(" · ")}</strong></li><li><span>Prior feedback</span><strong>{activeCoachContext.rejectedRecommendations.length ? `Rejected: ${activeCoachContext.rejectedRecommendations.join(", ")}` : "No rejected ideas yet"}</strong></li></ul><button>Edit saved club information</button></section><section className="askCoach"><p className="eyebrow dark">ASK YOUR COACH</p><h3>Use the full club context</h3><div className="questionChips">{["What should we focus on this month?", "Give us event ideas under $200.", "What can our secretary take over?"].map((question) => <button key={question} onClick={() => setCoachGoal(question)}>{question}</button>)}</div><textarea value={coachGoal} onChange={(event) => setCoachGoal(event.target.value)} placeholder="Ask a club-specific question…" /><button onClick={() => setCoachResponse(activeCoachOutput.coachAnswer)}>Ask coach →</button>{coachResponse && <div className="coachAnswer"><small>DEMO AI RESPONSE</small><p>{coachResponse}</p></div>}</section></aside>
        </div>
        ...
        <div className="clubStatusFolders" aria-label="Club status filters">{["All clubs", "Active", "Needs attention", "Restarting", "Inactive", "Archived", "Awaiting constitution", "Awaiting advisor", "Officer vacancies", "Financial follow-up"].map((filter) => <button className={staffClubFilter === filter ? "active" : ""} onClick={() => setStaffClubFilter(filter)} key={filter}><span>▰</span>{filter}<b>{filter === "All clubs" ? 28 : filter === "Active" ? 19 : filter === "Needs attention" ? 6 : filter === "Officer vacancies" ? 5 : 2}</b></button>)}</div>
