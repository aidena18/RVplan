  ...
  return (
    <main className={`app view-${view} coach-${coachMode}`}>
      ...
      {view === "home" ? <section className="homeWorkspace" aria-label="RVplan tools">
        ...
        {coachMode === "samples" && <div className="sampleBanner"><div><strong>Sample club profiles</strong><span>Explore different club situations and planning approaches.</span></div><button onClick={() => { setSetupStep(0); setCoachMode("setup"); }}>Set up my own club →</button></div>}
        <div className="coachTop"><button onClick={() => setView("home")}>← Back to tools</button><span>PERSONALIZED CLUB PLAN</span></div>
        ...
        <div className="coachShell">
          ...
          <div className="coachResult">
            <div className="aiResultHeader"><span>◎</span><div><small>YOUR LEADERSHIP PLAN</small><h3>{activeCoachContext.clubProfile}</h3></div><b>Personalized</b></div>
          ...
          <button className="coachChatLauncher" onClick={() => setCoachChatOpen((open) => !open)} aria-expanded={coachChatOpen}><span>◎</span><strong>{coachChatOpen ? "Close coach" : "Ask RVplan Coach"}</strong></button>
          {coachChatOpen && <aside className="coachSidePanel"><div className="chatHeader"><span>◎</span><div><strong>RVplan Coach</strong><small>Personalized for {activeCoachContext.clubProfile}</small></div><button onClick={() => setCoachChatOpen(false)} aria-label="Close coach">×</button></div><section className="askCoach"><div className="coachMessage">Hi! What would you like help with?</div><div className="questionChips">{["What should we focus on this month?", "Give us event ideas under $200."].map((question) => <button key={question} onClick={() => setCoachGoal(question)}>{question}</button>)}</div>{coachResponse && <div className="coachAnswer"><p>{coachResponse}</p></div>}<div className="chatComposer"><textarea value={coachGoal} onChange={(event) => setCoachGoal(event.target.value)} placeholder="Message your coach…" /><button onClick={() => setCoachResponse(activeCoachOutput.coachAnswer)} aria-label="Send message">→</button></div></section></aside>}
        </div>
        </>}
